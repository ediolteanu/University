...
void Tema3::SpawnNextWave() {
	if (platformsPositions.empty()) {
		...
		int xr = rand() % 3 - 1;
		if (dis(gen) == 10) {
			colectibles.push_back(glm::vec3(xr, 0.6f, last_platform.z));
...
void Tema3::Update(float deltaTimeSeconds)
{
	...
	//fundal
	{
		...
		modelMatrix = glm::scale(modelMatrix, glm::vec3(160, 90, 0.05f));
		apply_texture = 1;
...
void Tema3::FrameEnd()
{
	Text->RenderText("Score:" + std::to_string(score), 1190.0f, 10.0f, 1.0f, glm::vec3(1.0, 1.0, 1.0));
	//DrawCoordinatSystem();
}
