...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	glm::ivec2 resolution = window->GetResolution();
	if (translateX < 0 || translateX > resolution.y) signT *= -1;
	translateX += deltaTimeSeconds * signT * 100;

	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(150, 500);
	...
	scaleX = scaleY; // mentinem forma patrat
	if (scaleX > 3 || scaleX < 0.25) signS *= -1;
	scaleX += deltaTimeSeconds * 0.25 * signS;
	scaleY = scaleX; // mentinem forma patrat
	modelMatrix = glm::mat3(1);
