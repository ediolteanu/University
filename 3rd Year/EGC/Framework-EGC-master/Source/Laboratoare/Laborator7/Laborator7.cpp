...
void Laborator7::Update(float deltaTimeSeconds)
{
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab7"], modelMatrix, glm::vec3(0.4f, 0.4f, 0.4f));
	}
...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	GLint lightPositionLoc = glGetUniformLocation(shader->program, "light_position");
	glUniform3fv(lightPositionLoc, 1, glm::value_ptr(lightPosition));
	// TODO: Set eye position (camera position) uniform
	glm::vec3 eyePosition = GetSceneCamera()->transform->GetWorldPosition();
	GLint eyePositionLoc = glGetUniformLocation(shader->program, "eye_position");
	glUniform3fv(eyePositionLoc, 1, glm::value_ptr(eyePosition));
	// TODO: Set material property uniforms (shininess, kd, ks, object color) 
	GLint materialShininessLoc = glGetUniformLocation(shader->program, "material_shininess");
	glUniform1i(materialShininessLoc, materialShininess);

	GLint materialKdLoc = glGetUniformLocation(shader->program, "material_kd");
	glUniform1f(materialKdLoc, materialKd);

	GLint materialKsLoc = glGetUniformLocation(shader->program, "material_ks");
	glUniform1f(materialKsLoc, materialKs);
