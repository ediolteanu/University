...
int* get_dst(int rank, int numProcs, int leader) {
	...
	*/
	for (int i = 0; i < num_neigh; i++) {
		if (neigh[i] != v[rank]) {
			MPI_Recv(vRecv, numProcs, MPI_INT, neigh[i], 2, MPI_COMM_WORLD, &status);
...
int ** get_topology(int rank, int nProcesses, int * parents, int leader) {
	...
	for (int i = 0; i < num_neigh; i++) {
		if (parents[neigh[i]] == rank) {
			...
			for (int k = 0; k < nProcesses; k++){
				for (int l = 0; l < nProcesses; l++) {
					if (vTopology[k][l] != 0 && topology[k][l] == 0)
						topology[k][l] = vTopology[k][l];
				}
			}
	...
	if (rank != leader) {
		for (int i = 0; i < nProcesses; i++) {
			MPI_Send(topology[i], nProcesses, MPI_INT, parents[rank], 4, MPI_COMM_WORLD);
		}
	}
	...
	if (rank != leader) {
		for (int i = 0; i < nProcesses; i++)
			MPI_Recv(vTopology[i], nProcesses, MPI_INT, parents[rank], 4, MPI_COMM_WORLD, NULL);
		for (int k = 0; k < nProcesses; k++){
				for (int l = 0; l < nProcesses; l++) {
					topology[k][l] = vTopology[k][l];
				}
